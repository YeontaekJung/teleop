...
class ManusInspire(Node):
    ...
    def _start_calib(self):
        ...
        self.get_logger().warn("=== CALIBRATION 1/4: Open hands fully and hold... ===")

    def _reset_calib_samples(self):
        self._calib_samples = {
    ...
    def _srv_calibrate(self, req, resp):
        ...
        resp.message = (
            "Calibration started — "
            "1) open hands 4s, 2) close fists 4s, "
            "3) spread thumbs out 4s, 4) fold thumbs to palm 4s."
        )
        return resp
    ...
    def build_msg(self, pinky, ring, middle, index, thumb, spread):
        msg = InspireHandCtrl()
        msg.mode = 0b1101  # angle + force + speed
    ...
    def map_manus(self, ergo, hand='right'):
        ...
        spread_value = MAX_INSPIRE - int(spread_norm * MAX_INSPIRE)  # both hands inverted
